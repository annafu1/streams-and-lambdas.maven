...
public class StreamFilter {
    ...
    public StreamFilter() { // have to use random utils info to fill out the filter to check randomly the person w character a,z
    ...
    public List<Person> toListMultiLine() {
        return personStream.filter (
                p -> {
                    String fLetter = String.valueOf(p.getName().charAt(0)); //looking at starting character
                    return fLetter.equals(startingCharacter); //return true/false
                }
    ...
    public List<Person> toListOneLine() {
        return personStream.filter(p -> String.valueOf(p.getName().startsWith(startingCharacter)).equals(startingCharacter)).collect(Collectors.toList());
    }
    ...
    public Person[] toArrayMultiLine() {
        return personStream.filter(p -> {
                    String fLetter = String.valueOf(p.getName().charAt(0));
                    return fLetter.equals(this.startingCharacter);
                }
